...
def show_saved_list(tree):
    ...
    # Load data from CSV
    if os.path.exists('item_log.csv'):
        df = pd.read_csv('item_log.csv')
...
def log_image_details(img_id, title, file_path):
    ...
    # Append to CSV
    df.to_csv('image_log.csv', mode='a', header=not os.path.exists('image_log.csv'), index=False)
